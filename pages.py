  ...
  <div class="srv-panel" style="margin-top:13px">
    ...
    <div style="padding:20px 24px 24px;display:flex;flex-direction:column;gap:12px">
      ...
      <div style="display:flex;gap:8px">
        <button class="btn btn-p" id="clean-ip-add-btn" onclick="addCleanIp()" style="flex:1;justify-content:center;padding:10px"><i class="ti ti-plus"></i> افزودن آی‌پی</button>
        <button class="btn btn-o" id="clean-ip-cancel-btn" onclick="cancelEditCleanIp()" style="display:none;padding:10px 16px">انصراف</button>
      </div>
      ...
      if(allCleanIps.length){
        ...
        nlList.innerHTML=allCleanIps.map(x=>`
          <label style="display:flex;align-items:center;gap:8px;font-size:11px;cursor:pointer">
            <input type="checkbox" class="nl-cleanip-chk" value="${esc(x.id||x.ip)}" style="accent-color:var(--accent)">
  ...
  const cleanIpsHtml = allCleanIps.length ? `
    <div style="margin-top:10px;padding-top:10px;border-top:1px dashed var(--card-b)">
      ...
      <div style="display:flex;flex-direction:column;gap:6px">
        ${allCleanIps.map(x=>`
          <label style="display:flex;align-items:center;gap:8px;font-size:11px;cursor:pointer">
            <input type="checkbox" class="ns-cleanip-chk" data-proto="${proto}" value="${esc(x.id||x.ip)}" style="accent-color:var(--accent)">
            <span style="font-family:ui-monospace,monospace;direction:ltr">${esc(x.ip)}</span>
            ${x.label&&x.label!==x.ip?`<span style="color:var(--t3)">(${esc(x.label)})</span>`:''}
          </label>`).join('')}
      </div>
...
function renderCleanIpsList(){
  ...
  box.innerHTML=allCleanIps.map(x=>`
    <div style="display:flex;align-items:center;gap:9px;background:var(--card-2,rgba(0,0,0,.15));border:1px solid var(--card-b);border-radius:11px;padding:9px 12px">
      ...
      <button class="btn btn-sm btn-o btn-icon" onclick="startEditCleanIp('${esc(x.id||x.ip)}')" title="ویرایش (تغییر آدرس با حفظ همین برچسب)"><i class="ti ti-edit"></i></button>
      <button class="btn btn-sm btn-d btn-icon" onclick="deleteCleanIp('${esc(x.id||x.ip)}')" title="حذف"><i class="ti ti-trash"></i></button>
    </div>`).join('');
}
let editingCleanIpId=null;
function startEditCleanIp(id){
  const entry=allCleanIps.find(x=>(x.id||x.ip)===id);
  if(!entry)return;
  editingCleanIpId=id;
  document.getElementById('settings-clean-ip-val').value=entry.ip;
  document.getElementById('settings-clean-ip-label').value=entry.label&&entry.label!==entry.ip?entry.label:'';
  document.getElementById('clean-ip-add-btn').innerHTML='<i class="ti ti-device-floppy"></i> ذخیره تغییرات این آی‌پی';
  document.getElementById('clean-ip-cancel-btn').style.display='inline-flex';
  document.getElementById('settings-clean-ip-val').scrollIntoView({behavior:'smooth',block:'center'});
}
function cancelEditCleanIp(){
  editingCleanIpId=null;
  document.getElementById('settings-clean-ip-val').value='';
  document.getElementById('settings-clean-ip-label').value='';
  document.getElementById('clean-ip-add-btn').innerHTML='<i class="ti ti-plus"></i> افزودن آی‌پی';
  document.getElementById('clean-ip-cancel-btn').style.display='none';
}
async function addCleanIp(){
  ...
  try{
    let r;
    if(editingCleanIpId){
      r=await authF('/api/settings/clean-ips/'+encodeURIComponent(editingCleanIpId),{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({ip,label})});
    }else{
      r=await authF('/api/settings/clean-ips',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ip,label})});
    }
    const d=await r.json().catch(()=>({}));
    if(!r.ok)throw new Error(d.detail||'خطا');
    allCleanIps=d.clean_ips||[];
    renderCleanIpsList();
    const wasEditing=!!editingCleanIpId;
    cancelEditCleanIp();
    toast(wasEditing?'آی‌پی بروزرسانی شد ✓ — همه‌ی کانفیگ‌های وصل به این برچسب خودکار آدرس جدید رو می‌گیرن':'آی‌پی اضافه شد ✓','ok');
  }catch(e){toast('✗ '+(e.message||'خطا'),'err')}
}
async function deleteCleanIp(id){
  try{
    const r=await authF('/api/settings/clean-ips/'+encodeURIComponent(id),{method:'DELETE'});
    const d=await r.json().catch(()=>({}));
    if(!r.ok)throw new Error(d.detail||'خطا در حذف');
    allCleanIps=d.clean_ips||[];
    renderCleanIpsList();
    if(editingCleanIpId===id)cancelEditCleanIp();
